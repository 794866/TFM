...
def SobelAndCanny(imagen):
    ...
    def ordenar_puntos(pts):
        for pt in pts:
            n_puntos = np.concatenate([pts[pt]]).tolist()
        y_order = sorted(n_puntos, key=lambda n_puntos: n_puntos[1])
        x1_order = y_order[:2]
        x1_order = sorted(x1_order, key=lambda x1_order: x1_order[0])
        x2_order = y_order[2:4]
    ...
    for c in cnts:
        ...
        if len(approx) == 4:
            ...
            #print(centroidXCoordinate);print(centroidYCoordinate)
            #cv2.circle(img,(centroidXCoordinate,centroidYCoordinate),5,(0,255,0),-1)
            #cv2.imshow('circle', img)
            #x,y,w,h = cv2.boundingRect(c)
            #cv2.rectangle(img,(x,y),(x+w,y+h),(0,255,0),3)
            #print("rectangle", (x,y),(x+w,y+h))
        ...
        if len(approx) < 4:
            ...
            centroidXCoordinate = int(moment['m10'] / moment['m00'])
            centroidYCoordinate = int(moment['m01'] / moment['m00'])
            print(centroidXCoordinate);
            print(centroidYCoordinate)
            cv2.circle(img, (centroidXCoordinate, centroidYCoordinate), 5, (0, 255, 0), -1)
            # cv2.imshow('circle', img)
            x, y, w, h = cv2.boundingRect(c)
            cv2.rectangle(img, (x, y), (x + w, y + h), (0, 255, 0), 3)
